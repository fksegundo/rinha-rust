...
const testDataPath = __ENV.TEST_DATA || '/test/test-data.json';
...
const testData = new SharedArray('test-data', function () {
    return JSON.parse(open(testDataPath)).entries;
});
const statsArr = new SharedArray('test-stats', function () {
    return [JSON.parse(open(testDataPath)).stats];
});
...
export default function () {
    ...
    const apiUrl = __ENV.API_URL || 'http://localhost:9999/fraud-score';
    const res = http.post(
        apiUrl,
...
export function handleSummary(data) {
    ...
    const result = {
        ...
        latency: {
            p50: p50 ? p50.toFixed(2) + 'ms' : '0ms',
            p95: p95 ? p95.toFixed(2) + 'ms' : '0ms',
            p99: p99 ? p99.toFixed(2) + 'ms' : '0ms',
        },
        p99: p99 ? p99.toFixed(2) + 'ms' : '0ms',
        scoring: {
            breakdown: {
